...
- [x] **SEC-01**: URL fetcher blocks private, link-local, loopback, and cloud-metadata IPs (e.g. 169.254.169.254) including after DNS resolution
...
- [x] **SEC-04**: URL fetcher enforces a response-size cap and is decompression-bomb safe
...
- [x] **API-02**: `GET /audit/{job_id}` returns status, and on completion the 0–100 score + findings
- [x] **API-03**: `GET /audits` lists audit history with pagination
...
- [x] **API-06**: `GET /healthz` returns a deep health signal (DB reachable), not just process-up
- [x] **API-07**: Service exposes `/openapi.json` + `/docs` (Scalar) per global rule 21
- [x] **API-08**: A `callback_url` (webhook) on submit is notified on job completion, and the `callback_url` passes the SAME SSRF check as the audit URL
...
| SEC-01 | Phase 2: SSRF & Fetch Hardening | Complete |
| SEC-02 | Phase 2: SSRF & Fetch Hardening | Complete |
| SEC-03 | Phase 2: SSRF & Fetch Hardening | Complete |
| SEC-04 | Phase 2: SSRF & Fetch Hardening | Complete |
...
| API-02 | Phase 5: Bun+Hono API Layer | Complete (05-02) |
| API-03 | Phase 5: Bun+Hono API Layer | Complete (05-02) |
| API-04 | Phase 5: Bun+Hono API Layer | Complete (05-01) |
| API-05 | Phase 5: Bun+Hono API Layer | Complete (05-01) |
| API-06 | Phase 5: Bun+Hono API Layer | Complete (05-02) |
| API-07 | Phase 5: Bun+Hono API Layer | Complete (05-02) |
| API-08 | Phase 5: Bun+Hono API Layer | Complete (05-02) |
