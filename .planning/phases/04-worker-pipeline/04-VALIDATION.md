...
| 4-SCORE-04 | 1 | SCORE-04 | timeout→`SCORING_TIMEOUT`, 429→`SCORING_RATE_LIMITED`, 529/5xx→`SCORING_API_ERROR`, missing tool_use / zod-invalid→`SCORING_MALFORMED_OUTPUT` — **ALL retryable:true** (ROADMAP criterion #4); **`completeJob` call count == 0 on every error path** | unit (mock Anthropic) | `vitest run scorer` | ❌ W0 | ⬜ pending |
| 4-WORK-04d | 2 | SCORE-04/WORK-04 | retryable scoring failure with attempts<MAX → `requeueJob` (re-claimable); attempts≥MAX → `failJob` (terminal); `requeueJob`/`completeJob` false (stale lease) handled, no re-write; no unhandled rejection on `runAudit` throw | int+unit (PGlite + mock) | `vitest run worker pipeline` + `bun run --cwd packages/db test` | ❌ W0 | ⬜ pending |
